import React, {Component} from 'react';
import {
  View,
  Text,
  ImageBackground,
  TextInput,
  TouchableOpacity,
  Image,
  StatusBar,
} from 'react-native';

import {
  LoginButton,
  AccessToken,
  GraphRequest,
  GraphRequestManager,
} from 'react-native-fbsdk';
import styles from './indexCss';
import bgimg from '../../../assets/login.png';

import logo from '../../../assets/logo3.png';

export default class index extends Component {
  constructor() {
    super();
    //Setting the state for the data after login
    this.state = {
      user_name: '',
      token: '',
      profile_pic: '',
    };
  }

  get_Response_Info = (error, result) => {

    console.log('getting inside the function -----------------')
    if (error) {
      //Alert for the Error
      Alert.alert('Error fetching data: ' + error.toString());
    } else {
      //response alert
      alert(JSON.stringify(result));
      this.setState({ user_name: 'Welcome' + ' ' + result.name });
      this.setState({ token: 'User Token: ' + ' ' + result.id });
      this.setState({ profile_pic: result.picture.data.url });
    }
  };

  onLogout = () => {
    //Clear the state after logout
    this.setState({ user_name: null, token: null, profile_pic: null });
  };


  render() {
    return (
      <View style={styles.container}>
        <StatusBar hidden />
        <ImageBackground
          source={bgimg}
          style={styles.bgImgStyle}
          resizeMode="stretch">
          <View style={styles.contentView}>
            <Text style={styles.headerTextView}>Log in with Facebook</Text>
            <View style={styles.header}>
              <Image
                resizeMode="contain"
                source={logo}
                style={styles.imgView}
              />
             
            </View>

            <View style={styles.textView}>
                <Text style={styles.textContent}>The Lyfe will recieve</Text>
                <Text style={styles.textContent}>your public profile</Text>
            </View>


            <View style={styles.btnView}>
              {/* <TouchableOpacity
                style={styles.Btn}
                onPress={() => {
                  this.props.navigation.navigate('login');
                }}>
                <Text style={styles.BtnTxt}>Log in with Facebook</Text>
              </TouchableOpacity> */}
              {/* {this.state.avatar_url ? (
              <Image
                source={{uri: this.state.avatar_url}}
                style={styles.imageStyle}
              />
            ) : null} */}

{this.state.profile_pic ? (
          <Image
            source={{ uri: this.state.profile_pic }}
            style={styles.imageStyle}
          />
        ) : null}
        <Text style={styles.text}> {this.state.user_name} </Text>
        <Text> {this.state.token} </Text>

        <LoginButton
          readPermissions={['public_profile']}
          onLoginFinished={(error, result) => {
            if (error) {
              alert(error);
              alert('login has error: ' + result.error);
            } else if (result.isCancelled) {
              alert('login is cancelled.');
            } else {
              AccessToken.getCurrentAccessToken().then(data => {
                alert(data.accessToken.toString());

                const processRequest = new GraphRequest(
                  '/me?fields=name,picture.type(large)',
                  null,
                  this.get_Response_Info
                );
                // Start the graph request.
                new GraphRequestManager().addRequest(processRequest).start();
              });
            }
          }}
          onLogoutFinished={this.onLogout}
        />
            </View>
           
          </View>
        </ImageBackground>
      </View>
    );
  }
}
